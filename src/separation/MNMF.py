...
class MNMF(Base):
    ...
    def __init__(self, n_source=2, n_basis=2, init_SCM="unit", n_iter_init=15, interval_norm=10, xp=np, seed=0):
        """initialize MNMF
        ...
        -----------
            ...
            xp : numpy or cupy
        """
        super(MNMF, self).__init__(xp=xp, seed=seed, n_bit=64)
    ...
    def init_spatial_model(self):
        self.SCM_NFMM = self.xp.tile(self.xp.eye(self.n_mic), [self.n_source, self.n_freq, 1, 1]).astype(
            self.TYPE_COMPLEX
        )
        if "unit" in self.init_SCM:
            ...
            ave_power_FT = (self.xp.abs(self.X_FTM) ** 2).mean(axis=2)
            self.SCM_NFMM[0] = self.xp.einsum("fij, f -> fij", self.XX_FTMM.sum(axis=1), ave_power_FT.sum(axis=1))
            ...
            for n in range(self.n_source):
                self.SCM_NFMM[n] = self.xp.einsum(
                    "fi, fj -> fij",
            ...
            self.SCM_NFMM /= self.xp.trace(self.SCM_NFMM, axis1=2, axis2=3).real[:, :, None, None]
            self.SCM_NFMM += 1e-2 * self.xp.eye(self.n_mic)[None, None]
    ...
    def update_axiliary_variable(self):
        self.Yinv_FTMM = self.xp.linalg.inv(self.xp.einsum("nft, nfij -> ftij", self.PSD_NFT, self.SCM_NFMM))

        Yinv_x_FTM = self.xp.einsum("ftij, ftj -> fti", self.Yinv_FTMM, self.X_FTM)
        self.Yinv_X_Yinv_FTMM = self.xp.einsum("fti, ftj -> ftij", Yinv_x_FTM, Yinv_x_FTM.conj())
        self.tr_SCM_Yinv_X_Yinv_NFT = self.xp.trace(
            self.xp.einsum("nfij, ftjl -> nftil", self.SCM_NFMM, self.Yinv_X_Yinv_FTMM), axis1=-2, axis2=-1
        ).real
        self.tr_SCM_Yinv_NFT = self.xp.trace(
            self.xp.einsum("nfij, ftjl -> nftil", self.SCM_NFMM, self.Yinv_FTMM), axis1=-2, axis2=-1
        ).real

    def update_WH(self):
        W_numerator_NFK = self.xp.einsum("nkt, nft -> nfk", self.H_NKT, self.tr_SCM_Yinv_X_Yinv_NFT)
        W_denominator_NFK = self.xp.einsum("nkt, nft -> nfk", self.H_NKT, self.tr_SCM_Yinv_NFT)

        H_numerator_NKT = self.xp.einsum("nfk, nft -> nkt", self.W_NFK, self.tr_SCM_Yinv_X_Yinv_NFT)
        H_denominator_NKT = self.xp.einsum("nfk, nft -> nkt", self.W_NFK, self.tr_SCM_Yinv_NFT)

        self.W_NFK *= self.xp.sqrt(W_numerator_NFK / W_denominator_NFK)
        self.H_NKT *= self.xp.sqrt(H_numerator_NKT / H_denominator_NKT)

    def update_SCM(self):
        left_NFMM = self.xp.einsum("nft, ftij -> nfij", self.PSD_NFT, self.Yinv_FTMM)
        b = self.xp.einsum("nft, ftij -> nfij", self.PSD_NFT, self.Yinv_X_Yinv_FTMM)
        right_NFMM = (
            self.xp.einsum("nfij, nfjk, nfkl -> nfil", self.SCM_NFMM, b, self.SCM_NFMM)
            + (self.xp.eye(self.n_mic, dtype=self.TYPE_COMPLEX) * EPS)[None, None]
        )
        self.SCM_NFMM = geometric_mean_Ainv(left_NFMM, right_NFMM, xp=self.xp)
        self.SCM_NFMM = (self.SCM_NFMM + self.SCM_NFMM.transpose(0, 1, 3, 2).conj()) / 2

    def normalize(self):
        mu_NF = self.xp.trace(self.SCM_NFMM, axis1=-2, axis2=-1).real
    ...
    def calculate_log_likelihood(self):
        if not hasattr(self, "XX_FTMM"):
            self.XX_FTMM = self.xp.einsum("fti, ftj -> ftij", self.X_FTM, self.X_FTM.conj())
        Y_FTMM = self.xp.einsum("nft, nfij -> ftij", self.PSD_NFT, self.SCM_NFMM)
        return (
            -self.xp.trace(self.xp.linalg.inv(Y_FTMM) @ self.XX_FTMM, axis1=-2, axis2=-1).real
        ).sum() - self.xp.log(self.xp.linalg.det(Y_FTMM)).sum().real
...
def matrix_sqrth(A_NFMM, xp=np):
    eig_val_NFM, eig_vec_NFMM = xp.linalg.eigh(A_NFMM)
    eig_val_NFM[eig_val_NFM < EPS] = EPS
    return xp.einsum("nfij, nfj, nflj -> nfil", eig_vec_NFMM, xp.sqrt(eig_val_NFM), eig_vec_NFMM.conj())
