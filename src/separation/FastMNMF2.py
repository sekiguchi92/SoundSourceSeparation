...
class FastMNMF2(Base):
    ...
    def init_SCM(self):
        ...
        if "circular" in self.SCM:
            ...
            separater_init = FastMNMF2(
                n_source=self.n_source, n_basis=2, SCM=SCM_for_init, xp=self.xp, n_bit=self.n_bit, g_eps=self.g_eps,
            )
    ...
    def update_G(self):
        ...
        self.G_NM *= np.sqrt(numerator / denominator)
        self.calculate_Y()
    ...
    def load_param(self, filename):
        super(FastMNMF2, self).load_param(filename)
