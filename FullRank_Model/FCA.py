...
class FCA:
    ...
    def __init__(self, n_source=2, xp=np, init_SCM="unit", mode_update_parameter=["all", "one_by_one"][0], seed=0):
        """ initialize FCA
    ...
    def initialize_covarianceMatrix(self):
        ...
        if "unit" in self.init_SCM:
            ...
            MixingMatrix_FMM = self.xp.linalg.inv(ilrma.SeparationMatrix_FMM)
            separated_spec_power = self.xp.abs(ilrma.separated_spec).mean(axis=(1, 2))
            for n in range(self.n_source):
                self.covarianceMatrix_NFMM[n] = MixingMatrix_FMM[:, :, separated_spec_power.argmax(), None] @ MixingMatrix_FMM[:, None, :, separated_spec_power.argmax()].conj()
                separated_spec_power[separated_spec_power.argmax()] = -np.inf
            self.covarianceMatrix_NFMM /= self.xp.trace(self.covarianceMatrix_NFMM, axis1=2, axis2=3).real[:, :, None, None]
            self.covarianceMatrix_NFMM += 1e-2 * self.xp.eye(self.n_mic, dtype=self.xp.complex)[None, None]
        self.normalize()
    ...
    def make_filename_suffix(self):
        self.filename_suffix = f"S={self.n_source}-it={self.n_iteration}-init={self.init_SCM}-update={self.mode_update_parameter}"
    ...
    def update_axiliary_variable(self):
        self.Yinv_FTMM = self.xp.linalg.inv( (self.lambda_NFT[..., None, None].astype(self.xp.complex) * self.covarianceMatrix_NFMM[:, :, None]).sum(axis=0) )
    ...
    def update_lambda(self):
        self.lambda_NFT *= self.xp.sqrt(self.tr_Cov_Yinv_X_Yinv_NFT / self.tr_Cov_Yinv_NFT) + EPS
    ...
    def update_covarianceMatrix(self):
        ...
        b_1 = self.covarianceMatrix_NFMM @ (self.lambda_NFT[..., None, None] * self.Yinv_X_Yinv_FTMM[None]).sum(axis=2) @ self.covarianceMatrix_NFMM + (self.xp.eye(self.n_mic, dtype=self.xp.complex) * EPS)[None, None] # N F M M
    ...
    def calculate_log_likelihood(self):
        Yinv_FTMM = self.xp.linalg.inv((self.lambda_NFT[..., None, None] * self.covarianceMatrix_NFMM[:, :, None]).sum(axis=0))
    ...
    def separate_WienerFilter(self, mic_index=MIC_INDEX): # separate using Wiener filter
        ...
        Omega_sum_inv_FTMM = self.xp.linalg.inv(Omega_NFTMM.sum(axis=0)) # F T M M
...
if __name__ == "__main__":
    ...
    parser.add_argument(                '--n_fft', type= int, default=  1024, help='number of frequency bins')
    parser.add_argument(                '--n_mic', type= int, default=     8, help='number of microphones')
    parser.add_argument(             '--n_source', type= int, default=     2, help='number of noise sources')
    parser.add_argument(          '--n_iteration', type= int, default=    30, help='number of iterations')
    ...
    separater = FCA(n_source = args.n_source, xp=xp, init_SCM=args.init_SCM, mode_update_parameter=args.mode_update_parameter, seed=0)
    ...
    print(separater.lambda_NFT.dtype, separater.covarianceMatrix_NFMM.dtype)