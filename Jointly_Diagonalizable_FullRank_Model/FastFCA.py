...
class FastFCA:
    ...
    def initialize_covarianceMatrix(self):
        if ("unit" in self.init_SCM) or ("circular" in self.init_SCM):
            ...
            mixture_covarianceMatrix_FMM = (
                self.XX_FTMM.sum(axis=1) / (self.xp.trace(self.XX_FTMM, axis1=2, axis2=3).sum(axis=1))[:, None, None]
            )
            ...
            ilrma.solve(
                n_iteration=15, save_likelihood=False, save_wav=False, save_path="./", interval_save_parameter=1000
            )
    ...
    def solve(
        ...
        for it in tqdm(range(self.n_iteration)):
            ...
            if save_parameter and ((it + 1) % interval_save_parameter == 0) and ((it + 1) != self.n_iteration):
                self.save_parameter(
                    save_path + "{}-parameter-{}-{}.pic".format(self.method_name, self.filename_suffix, it + 1)
                )
            ...
            if save_wav and ((it + 1) % interval_save_parameter == 0) and ((it + 1) != self.n_iteration):
                ...
                self.save_separated_signal(
                    save_path + "{}-sep-Wiener-{}-{}.wav".format(self.method_name, self.filename_suffix, it + 1)
                )
        ...
        if save_likelihood:
            ...
            pic.dump(
                ...
                open(
                    save_path
                    + "{}-likelihood-interval={}-{}.pic".format(
                        self.method_name, interval_save_parameter, self.filename_suffix
                    ),
                    "wb",
                ),
            )
    ...
    def udpate_Diagonalizer(self):
        for m in range(self.n_mic):
            ...
            self.Q_FMM[:, m] = (
                tmp_FM / self.xp.sqrt(((tmp_FM.conj()[:, :, None] * V_FMM).sum(axis=1) * tmp_FM).sum(axis=1))[:, None]
            ).conj()
    ...
    def calculate_log_likelihood(self):
        return (
            ...
            + self.n_time
            * np.log(
                np.linalg.det(self.convert_to_NumpyArray(self.Q_FMM @ self.Q_FMM.conj().transpose(0, 2, 1)))
            ).sum()
            - self.xp.log(self.Y_FTM).sum()
        ).real - self.n_mic * self.n_freq * self.n_time * np.log(np.pi)

    def calculate_covarianceMatrix(self):
        covarianceMatrix_NFMM = self.xp.zeros(
            [self.n_source, self.n_freq, self.n_mic, self.n_mic], dtype=self.TYPE_COMPLEX
        )
        diagonalizer_inv_FMM = self.xp.linalg.inv(self.Q_FMM)
        for n in range(self.n_source):
            for f in range(self.n_freq):
                covarianceMatrix_NFMM[n, f] = (
                    diagonalizer_inv_FMM[f] @ np.diag(self.G_NFM[n, f]) @ diagonalizer_inv_FMM[f].conj().T
                )
    ...
    def separate_FastWienerFilter(self, source_index=None, mic_index=MIC_INDEX):
        ...
        if source_index != None:
            self.separated_spec = self.convert_to_NumpyArray(
                (
                    ...
                    @ (
                        Qx_FTM
                        * (
                            (self.lambda_NFT[source_index, :, :, None] * self.G_NFM[source_index, :, None])
                            / (self.lambda_NFT[..., None] * self.G_NFM[:, :, None]).sum(axis=0)
                        )
                    )[..., None]
                )[:, :, mic_index, 0]
            )
            ...
            for n in range(self.n_source):
                tmp = self.convert_to_NumpyArray(
                    (
                        diagonalizer_inv_FMM[:, None]
                        @ (
                            Qx_FTM
                            * (
                                (self.lambda_NFT[n, :, :, None] * self.G_NFM[n, :, None])
                                / (self.lambda_NFT[..., None] * self.G_NFM[:, :, None]).sum(axis=0)
                            )
                        )[..., None]
                    )[:, :, mic_index, 0]
